...
public abstract class Repository {
    ...
    protected final <T> T executeSelect(String sql, PreparedStatementSetter pss, ResultSetMapper<T> mapper) {
    	PreparedStatement ps = TransactionSynchronizationManager.current(database).read(sql);
        try {
            pss.set(ps);
    ...
    protected final <E> void executeBatchInsert(String sql, InsertMapper<E> im, List<E> pojos,
        ...
        PreparedStatement ps =  TransactionSynchronizationManager.current(database).write(sql);
        for (E po : pojos) {
            try {
                im.insert(ps, po);
            } catch (SQLException cause) {
    ...
    protected final <T> Stream<T> stream(String sql, PreparedStatementSetter pss, ResultSetMapper<T> mapper) {
    	...
    	TransactionContext tc = TransactionSynchronizationManager.current(database);
