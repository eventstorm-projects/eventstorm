...
public final class QueryJacksonStdSerializerGenerator {
    ...
    public void generate(ProcessingEnvironment processingEnvironment, SourceCode sourceCode) {
        ...
        try (Logger logger = Logger.getLogger(processingEnvironment, "eu.eventstorm.event.query", "QueryJacksonStdSerializerGenerator")) {
            this.logger = logger;
        }
    ...
    private void writeMethod(Writer writer, QueryDescriptor ed) throws IOException {
        ...
        for (QueryPropertyDescriptor epd : ed.properties()) {
            ...
            } else if (Date.class.getName().equals(epd.getter().getReturnType().toString())) {
                writeDate(writer, epd);
    ...
    private void writeDate(Writer writer, QueryPropertyDescriptor epd) throws IOException {
        if (isNullable(epd)) {
            writer.write("        if (payload." + epd.getter().getSimpleName() + "() != null) {");
            writeNewLine(writer);
