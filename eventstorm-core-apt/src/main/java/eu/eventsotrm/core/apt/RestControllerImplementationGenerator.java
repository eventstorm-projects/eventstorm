...
final class RestControllerImplementationGenerator {
	...
	private static void writeHeader(Writer writer, ProcessingEnvironment env, ImmutableList<RestControllerDescriptor> desc)
	        throws IOException {
	...
	private static void writeSpring(Writer writer, RestControllerDescriptor rcd) throws IOException {
		...
		if (HttpMethod.PUT == rcd.getRestController().method()) {
			writer.write("    @org.springframework.web.bind.annotation.PutMapping(name=\"" + rcd.getRestController().uri() +"\")");
			writeNewLine(writer);
			return;
		}
