...
import eu.eventstorm.cqrs.QueryDescriptorException;
...
public final class QueryDescriptorGenerator {
	...
	private static void writeHeader(Writer writer, String pack, DatabaseQueryDescriptor descriptor) throws IOException {
		...
		writeNewLine(writer);
		writer.write("import " + QueryDescriptorException.class.getName() + ";");
		writeNewLine(writer);
	...
	private void writeMethodExpression(Writer writer, DatabaseQueryDescriptor ed) throws IOException {
		...
		writeNewLine(writer);
		writer.write("        Function<String, PreparedStatementIndexSetter> function = PREPARED_STATEMENT_INDEX_SETTERS.get(property);");
		writeNewLine(writer);
		writer.write("        if (function == null) {");
		writeNewLine(writer);
		writer.write("            throw new QueryDescriptorException(QueryDescriptorException.Type.PROPERTY_NOT_FOUND, ImmutableMap.of(\"property\",property));");
		writeNewLine(writer);
		writer.write("        }");
		writeNewLine(writer);
		writer.write("        return function.apply(value);");
		writeNewLine(writer);
		writer.write("    }");
		writeNewLine(writer);
	}
